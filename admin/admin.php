...
function wpcf7_admin_menu() {
	...
	$edit = add_submenu_page( 'wpcf7',
		__( 'Edit Contact Form', 'wpcf7' ), __( 'Contact Forms', 'wpcf7' ),
		'wpcf7_read_contact_forms', 'wpcf7', 'wpcf7_admin_management_page' );

	add_action( 'load-' . $edit, 'wpcf7_load_contact_form_admin' );

	$addnew = add_submenu_page( 'wpcf7',
		__( 'Add New Contact Form', 'wpcf7' ), __( 'Add New', 'wpcf7' ),
		'wpcf7_edit_contact_forms', 'wpcf7-new', 'wpcf7_admin_add_new_page' );
...
<p><a href="<?php echo esc_url( menu_page_url( 'wpcf7-new', false ) ); ?>" class="button" /><?php echo esc_html( __( 'Add New', 'wpcf7' ) ); ?></a></p>
...
function wpcf7_welcome_panel() {
	...
	$option = (array) get_user_meta( get_current_user_id(),
		'wpcf7_show_welcome_panel', true );

	if ( in_array( WPCF7_VERSION, $option ) ) {
		$classes .= ' hidden';
	}
?>
<div id="welcome-panel" class="<?php echo esc_attr( $classes ); ?>">
	<?php wp_nonce_field( 'wpcf7-welcome-panel-nonce', 'welcomepanelnonce', false ); ?>
	<a class="welcome-panel-close" href="<?php echo esc_url( menu_page_url( 'wpcf7', false ) ); ?>"><?php echo esc_html( __( 'Dismiss', 'wpcf7' ) ); ?></a>
...
function wpcf7_admin_ajax_welcome_panel() {
	...
	if ( empty( $_POST['visible'] ) ) {
		$option[] = WPCF7_VERSION;
	}

	$option = array_unique( $option );

	update_user_meta( get_current_user_id(), 'wpcf7_show_welcome_panel', $option );
