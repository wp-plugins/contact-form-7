...
function wpcf7_file_validation_filter( $result, $tag ) {
	...
	if ( $file['error'] && UPLOAD_ERR_NO_FILE != $file['error'] ) {
		$result['valid'] = false;
		$result['reason'] = array(
			$name => wpcf7_get_message( 'upload_failed_php_error' ) );
		return $result;
	}
	...
	if ( ! preg_match( $file_type_pattern, $file['name'] ) ) {
		$result['valid'] = false;
		$result['reason'] = array(
			$name => wpcf7_get_message( 'upload_file_type_invalid' ) );
		return $result;
	}
	...
	if ( $file['size'] > $allowed_size ) {
		$result['valid'] = false;
		$result['reason'] = array(
			$name => wpcf7_get_message( 'upload_file_too_large' ) );
		return $result;
	}
	...
	if ( false === @move_uploaded_file( $file['tmp_name'], $new_file ) ) {
		$result['valid'] = false;
		$result['reason'] = array(
			$name => wpcf7_get_message( 'upload_failed' ) );
		return $result;
	}
